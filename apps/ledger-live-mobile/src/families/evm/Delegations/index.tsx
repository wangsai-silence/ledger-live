...
import DelegationLabelRight from "~/families/cosmos/Delegations/LabelRight";
...
function ValidatorBadge({ name, size }: { name: string; size: number }) {
  const { colors } = useTheme();
  const initial = name.trim().slice(0, 1).toUpperCase() || "?";
  return (
    <Circle size={size} bg={rgba(colors.live, 0.16)}>
      <LText semiBold color="live" style={{ fontSize: Math.max(12, size / 3) }}>
        {initial}
      </LText>
    </Circle>
  );
...
function DelegationRow({ delegation, currency, onPress, isLast = false }: RowProps) {
  const { colors } = useTheme();
  const { t } = useTranslation();
  const { validator, validatorAddress, formattedAmount, amount } = delegation;
  const validatorName = validator?.name ?? validatorAddress;

  return (
    <TouchableOpacity
      style={[
        styles.row,
        styles.delegationRow,
        !isLast ? { ...styles.borderBottom, borderBottomColor: colors.lightGrey } : undefined,
      ]}
      onPress={() => onPress(delegation)}
    >
      <View style={styles.icon}>
        <ValidatorBadge size={42} name={validatorName} />
      </View>
    ...
    <View style={styles.root}>
      ...
      {delegations.length === 0 ? (
        <AccountDelegationInfo
          ...
          infoUrl={urls.ledgerAcademy.ethereumStakingHowToStakeEth}
