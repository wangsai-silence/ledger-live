...
import { TZKT_MOCK_URL } from "./fixtures";
...
async function indexBlock(level: number, watchedAddress: string): Promise<void> {
  ...
  for (const opGroup of ops) {
    for (const content of opGroup.contents ?? []) {
      ...
      if (content.kind === "transaction" && isRelevant) {
        trackedOps.push({
          id: opIdCounter++,
          hash: opGroup.hash,
          block: blockHash,
      ...
      } else if (content.kind === "reveal" && isRelevant) {
        trackedOps.push({
          id: opIdCounter++,
          hash: opGroup.hash,
          block: blockHash,
      ...
      } else if (content.kind === "delegation" && content.source === watchedAddress) {
      ...
      return HttpResponse.json([
        {
          ...
          fees: 0,
          nonceRevealed: false,
          proposer: { address: "tz1VSUr8wwNhLAzempoch5d6hLRiTh8Cjcjb" },
          producer: { address: "tz1VSUr8wwNhLAzempoch5d6hLRiTh8Cjcjb" },
          ...
          bonus: 0,
          priority: 0,
          baker: { address: "tz1VSUr8wwNhLAzempoch5d6hLRiTh8Cjcjb" },
          lbEscapeVote: false,
          lbEscapeEma: 0,
        },
      ]);
