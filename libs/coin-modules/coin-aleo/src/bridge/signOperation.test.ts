...
describe("buildSignOperation", () => {
  ...
  it("should call signNestedCall with the buffer derived from the nested call tlv", async () => {
    const nestedTlv = "deadbeef";
    const requestWithNested = getMockedPreparedRequestResponse({
      nested_calls: [getMockedPreparedRequestResponse({ is_root: false, tlv: nestedTlv })],
    });
    ...
    expect(mockSigner.signNestedCall).toHaveBeenCalledTimes(1);
    expect(mockSigner.signNestedCall).toHaveBeenCalledWith(Buffer.from(nestedTlv, "hex"));
  });
