...
describe("getBlock", () => {
  ...
  describe("zkSync L1→L2 priority transactions (receipt type 0xff)", () => {
    ...
    it("prepends a credit op (peer=L2BaseToken) and keeps native + 800A self pairs", async () => {
      buildZksyncMocks(
        { from: USER, to: USER, value: "270000000000000000" },
        {
          type: 0xff,
    ...
    it("does not prepend a credit op when 800A pair is absent (type 0xff)", async () => {
      buildZksyncMocks(
        { from: USER, to: USER, value: "270000000000000000" },
